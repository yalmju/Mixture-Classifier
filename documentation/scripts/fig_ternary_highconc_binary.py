...
n_b = int((z["k"] == 2).sum()); n_h = int((z["k"] == 3).sum())
...
def draw(points):
    ...
    note = (f"held-out (deployed model) · every binary condition (n={n_b}) + "
            f"ternary with total ≥ 99 µM (n={n_h})")
    if points:
    ...
    OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                       "..", "figures")
    for ext in ("png", "pdf"):
        fig.savefig(os.path.join(OUT, f"fig_ternary_highconc_binary{tag}.{ext}"),
