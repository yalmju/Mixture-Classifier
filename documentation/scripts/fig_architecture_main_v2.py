...
def sub_grid(x0, y0, w, color, seed=7, frac=0.45):
    """한 성분의 '분포 지도' 썸네일: 배경 회색, 검출 픽셀은 성분색(세기 = 진하기)."""
    from matplotlib.colors import to_rgb
    rng = np.random.default_rng(seed)
    vals = rng.uniform(0.0, 1.0, (6, 6))
    on = rng.uniform(0, 1, (6, 6)) < frac
    cw = w / 6
    ch = cw * AR
    ...
    for r in range(6):
        for c in range(6):
            if on[r, c]:
                t = 0.45 + 0.55 * vals[r, c]
                fc_ = tuple(1 - t * (1 - base))
            else:
                fc_ = "#e6e8eb"
            ov.add_patch(plt.Rectangle((x0 + c * cw, y0 + r * ch), cw * 0.9,
                                       ch * 0.9, facecolor=fc_,
...
out_tag(0.688, 0.915)
# distribution mapping — per-substance pixel maps (top) + map composition pie (bottom)
cap(0.563, 0.795, "distribution mapping", fs=BOX_T, col=INK, weight="bold")
for k, s_ in enumerate(SUBS):
    gx0 = 0.516 + k * 0.032
    cap(gx0 + 0.013, 0.735, s_, fs=6.2, col=CO[s_], weight="bold")
    sub_grid(gx0, 0.575, 0.026, CO[s_], seed=11 + k, frac=0.40)
axp = fig.add_axes([0.512, 0.115, 0.050, 0.050 * AR], aspect="equal")
axp.pie([34, 32, 28, 6], colors=[CO["THI"], CO["TBZ"], CO["DQ"], "#9aa3ad"],
        startangle=90, counterclock=False,
        wedgeprops=dict(edgecolor="white", linewidth=0.7))
axp.set_axis_off()
cap(0.598, 0.375, "map composition (%)", fs=6.4, col=INK, weight="bold", ha="left")
for i, (s_, v) in enumerate(zip(["THI", "TBZ", "DQ"], [34, 32, 28])):
    cap(0.600, 0.305 - i * 0.062, f"{s_}   ~{v}", fs=6.2, col=CO[s_], ha="left",
        weight="bold")
cap(0.600, 0.118, "BLK  ~6 (removed)", fs=5.8, col="#8a919b", ha="left")
cap(0.563, 0.055, "absent component → 0  (absence gate, P < 0.2)", fs=SUB_FS)
arrow(0.612, CY, 0.726, CY)
cap(0.668, CY - 0.075, "gated pixels · composition", fs=6.2, col=INK)
...
out_tag(0.958, 0.915)
ov.add_patch(FancyBboxPatch((0.740, 0.180), 0.236, 0.665,
                            boxstyle="round,pad=0.002,rounding_size=0.008",
                            facecolor="#fdf7ee", edgecolor="#d9a441", lw=1.0,
                            zorder=0.6))
...
for (yy, s_, c_) in zip(np.linspace(CY + 0.045, CY - 0.045, 3), SUBS,
    ...
    ov.text(0.908, yy, s_, fontsize=6.2, color=c_, ha="left", va="center",
            weight="bold", zorder=4)
cap(0.858, 0.752, "12 map features: log10 Ccal ·3 · composition ·3 · log1p band ·3 · log-total p10/50/90",
    fs=5.2)
cap(0.858, 0.205, "C = Ccal · 10^Δ  ·  Ccal = log-linear band calibration (9–144 µM)",
    fs=5.4, col=PLAIN)
box(0.746, 0.100, 0.224, 0.060,
    [("domain check: nearest training map z-distance ≤ 3, else refused", "s")],
    ls="--", lw=0.9, edge="#d9a441")
box(0.746, 0.025, 0.224, 0.060,
    [(r"declared total (optional):  $\hat{C}_i$ = $\bar{p}_i$ · C$_{total}$", "s")],
    ls="--", lw=0.9)

OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "figures")
os.makedirs(OUT, exist_ok=True)
for ext in ("png", "pdf"):
    fig.savefig(os.path.join(OUT, f"fig_architecture_main_v2.{ext}"), dpi=600,
                bbox_inches="tight", pad_inches=0.02, facecolor="white")
print("saved fig_architecture_main_v2.png/.pdf")
