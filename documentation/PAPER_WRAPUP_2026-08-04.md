...
> 🔴 **전 구간 공통 제약 — 반복측정이 없습니다(n = 1).**
> 조성·농도·K·LOD 어느 것에도 측정 반복이 없어, **보고된 ± 는 전부 모델 시드 산포이지
> 측정 재현성이 아닙니다.** K 8.7배, 조성 19.2%, LOD 값 모두 신뢰구간이 없습니다.
> 이 상태로는 SANTE RSD도 계산 불가입니다. 결과를 읽을 때 이 전제를 먼저 적용하고,
> 논문에도 결과 옆에 명시해야 합니다. → 5절 남은 실험 1순위.
...
| 물질 | 평균 표면 **신호** 점유 | 평균 용액 분율 | 표면/용액 |
|---|---|---|---|
| THI | 56.5% | 33.3% | **1.69× 과대** |
| TBZ | 26.8% | 33.3% | 0.80× |
| DQ | 16.7% | 33.3% | **0.51× 과소** |
...
실측 표면 신호비는 56.5 / 16.7 = **3.4배**. **같은 방향이고 같은 자릿수**다.
...
K의 상대값 — 기판 gain에 불변이고 readout 두 가지에서 일치한다. → 3b절
...
| 순물질 레퍼런스 | DQ · TBZ · THI · BLK, 물질당 3배치 (`Pure/samples.csv`) |
