...
def apply_recovery(model, items, progress=None):
    ...
    for k, it in enumerate(items):
        ...
        nd_frac = nd = None
        if res.get("uM"):
            # 표시층 ND 플래그 (표면 근거): presence 게이트와 별개의 증거 계층 —
            # NNLS 표면 분율 < 4% 면 µM 을 <LOD 로 표기하라는 신호. 값은 건드리지
            # 않는다 (게이트가 지운 성분은 이미 0). UM_ND_FLAG_2026-08-28.md 참조.
            if det_px is None:
        ...
        if res.get("uM"):
            ...
            if nd is not None:
                row["surface_nd"] = {sn: bool(nd.get(sn, False)) for sn in subs}
