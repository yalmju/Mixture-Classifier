...
def _map_spectra(cube, mask, n_px=0, spread=False, baseline_correct=True):
    ...
    mean = wn_ @ cube
    out = [np.clip(mean - als_baseline(mean), 0, None) if baseline_correct
    ...
    if test_items:
        ...
        for k, it in enumerate(test_items):
            ...
            if nnls_screen:
                ...
                _w, cube, _m, _c = load_map(it[0]); local_mask = mask
            ya = _mean_spectrum(cube, local_mask, baseline_correct=not nnls_screen)
...
def apply_recovery(model, items, progress=None):
    ...
    for k, it in enumerate(items):
        ...
        if key in held:                                  # held-out composition from training
            ...
            res = {"uM": None}
            # The concentration head is independent of the composition held-out cache.
            # For a genuinely independent Role=test map, evaluate it on the same NNLS
            # hit pixels and report robust median/P10/P90 summaries.
            if model.get("nnls_screen") and model.get("uM"):
