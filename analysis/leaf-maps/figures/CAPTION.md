...
성분 정식명은 프로젝트에서 쓰는 표기와 맞출 것. 아래 "thiophanate-methyl" 은 THI 의
자리표시자다 — 실제 화합물명으로 바꿔 쓸 것.

---

## Fig_leaf_maps

**Raman maps of the leaf-surface series.** Each panel is one map, plotted after
...
cover different areas (450 µm for the 10 × 10 maps, 1900 µm for the 20 × 20 maps).
Columns are grouped as single-analyte preparations (diquat DQ, thiabendazole TBZ,
thiophanate-methyl THI), the ink-only control, and the mixture series (as applied, and
after peeling). **Top row**, integrated intensity of the ink reporter band in the
...
seven maps within a row (limits given by the colour bar) but not between rows.

## Fig_leaf_spectra

**Representative spectra of the leaf-surface series.** (**a**) Median spectrum of the
...
the threshold. The single-analyte THI preparation reaches 71 % for THI against 2 % for
DQ and TBZ, and all three analytes clear the threshold in the mixture map.
...
>
> 검출률을 혼합 3맵에 나란히 쓸 때 — `leavesmix` · `leavesmix2peel` 은 1900 µm 변,
> `leavesmix2peel2` 는 450 µm 변이라 마지막 맵이 보는 면적이 1/18 이다. 캡션에 밝히거나
> 같은 면적으로 다시 잴 것.
