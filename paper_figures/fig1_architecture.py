...
txt(gx + gw/2, YC + 20, "23,125 hit pixels", 9.5)
...
for x, w, h, lab in ((bx + 8, 32, 156, "1,290"), (bx + 84, 28, 114, "256"), (bx + 152, 24, 82, "64")):
    ...
    txt(x + w/2 + 3.5, YC + 96, lab, 10.5, INK, "middle", 700)
arrow(bx + 44, YC, bx + 80, YC); arrow(bx + 116, YC, bx + 148, YC)
txt(bx + 100, YC - 100, "BN · ReLU · dropout 0.15", 9.5)
...
for i, (col, lab) in enumerate(((DQ, "DQ"), (TBZ, "TBZ"), (THI, "THI"), (BLK, "BLK"))):
    ...
    txt(smx + smw/2, yy + 15, lab, 9, "#ffffff", "middle", 700)
arrow(bx + 184, YC, smx - 4, YC)
txt(smx + smw/2, YC + 96, "softmax 4", 10.5, INK, "middle", 700)
txt(bx + 120, YC + 122, "L1 per pixel against the map's label ratio · minor components ×3", 9.5)
...
txt(rx + cw/2, ry - 6, "mean pool → composition (%)", 10, INK, "middle", 600)
...
txt((smx + smw + cx0) / 2 + 4, YC - 13, "the 3 compound ratios, renormalised", 9.5)
...
txt(ctx_x + ctx_w/2, YC + 96, "16-D context", 10.5, INK, "middle", 700)
txt(ctx_x - 4, YC + 110, "3 ratios · 1 intensity · 9 + 3 map quantiles (P10/50/90)", 9.5, MUTE, "start")

nx = ctx_x + 92
stack(nx, 112, 26, CFILL, CEDGE); txt(nx + 16.5, YC + 96, "128", 10.5, INK, "middle", 700)
stack(nx + 70, 78, 22, CFILL, CEDGE); txt(nx + 84, YC + 96, "32", 10.5, INK, "middle", 700)
arrow(ctx_x + ctx_w + 6, YC, nx - 4, YC); txt(ctx_x + ctx_w + 30, YC - 13, "z-score", 9.5)
arrow(nx + 34, YC, nx + 66, YC)
txt(nx + 46, YC - 78, "BN · ReLU · dropout 0.25", 9.5)
...
txt(lx + (3*lw + 6)/2, YC + 96, "log₁₀(µM) × 3", 10.5, INK, "middle", 700)
txt(lx + (3*lw + 6)/2, YC + 122, "smooth-L1 on the per-map median", 9.5)

mo = lx + 3*lw + 6 + 76                                     # readout 2 — median pool
arrow(lx + 3*lw + 12, YC, mo - 10, YC)
txt((lx + 3*lw + 12 + mo - 10) / 2, YC - 13, "median pool", 9.5)
for i, (col, lab, f) in enumerate(((DQ, "DQ", .78), (TBZ, "TBZ", .52), (THI, "THI", 1.0))):
    ...
    txt(mo + i*25 + 9, YC + 43, lab, 9, INK, "middle", 600)
add(f'<line x1="{mo-4}" y1="{YC+30}" x2="{mo+3*25}" y2="{YC+30}" stroke="{MUTE}" stroke-width="0.9"/>')
txt(mo + 34, YC - 46, "µM per map", 10, INK, "middle", 600)

# -------------------------------------------------------------------- footer
add(f'<line x1="24" y1="356" x2="{W-24}" y2="356" stroke="#d7dee6" stroke-width="1"/>')
txt(W/2, 374, "Leave-one-condition-out · 71 distinct compositions, the model refit once for each", 10.5, INK, "middle", 600)
txt(W/2, 388, "every map of a held-out composition leaves together, so a repeat measurement is never scored against its own twin", 9.5)
add('</svg>')
