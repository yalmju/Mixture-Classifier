...
def test_test_role_map_gets_uM_pred():
    ...
    # 2026-08-28 — documentation/UM_ND_FLAG_2026-08-28.md). Absent components
    # must be flagged; a dominant present component must not be. The µM values
    # themselves must stay un-gated.
    nd = row.get("uM_nd")
    assert nd is not None, "uM_nd flag missing from recovery row"
    ...
    assert all(v > 0 for v in row["uM_pred"].values()), \
        "µM values were gated — the ND flag must be display-only"
